...
public class SpawnShadow : MonoBehaviour
{
    ...
    public void Setup(float distance, float SpawnTimeBase, float baseScale = 1f)
    {
        SpawnTime = SpawnTimeBase * Time.deltaTime + Time.time;
        transform.localScale = new Vector3(Mathf.Lerp(baseScale * 1.1f, baseScale * 0.6f, distance / 200), Mathf.Lerp(0.3f, 0.6f, distance / 1000), Mathf.Lerp(baseScale * 1.1f, baseScale * 0.6f, distance / 200));
        distance *= 0.7f;
